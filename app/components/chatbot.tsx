    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-48 right-6 z-50 w-[340px] sm:w-[380px] max-h-[560px] bg-white rounded-2xl shadow-2xl border border-[#eae6de] flex flex-col overflow-hidden">
          {/* Header */}
          <div className="bg-[#101F46] px-5 py-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-amber-400 flex items-center justify-center font-bold text-[#101F46] text-sm">
                K
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-[#f7f5f0]">
            {messages.map((msg) => (
              ...
              >
                {msg.role === "bot" && (
                  <div className="w-7 h-7 rounded-full bg-amber-400 flex items-center justify-center font-bold text-[#101F46] text-xs mr-2 mt-1 shrink-0">
                    K
                  </div>
                )}
                <div
                  className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-line ${
                    msg.role === "user"
                      ? "bg-[#101F46] text-white rounded-tr-sm"
            ...
            {typing && (
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-amber-400 flex items-center justify-center font-bold text-[#101F46] text-xs shrink-0">
                  K
                </div>
          ...
          <div className="px-4 py-3 bg-white border-t border-[#eae6de] flex gap-2 shrink-0">
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl bg-[#101F46] text-white flex items-center justify-center hover:bg-[#162d54] disabled:opacity-40 transition shrink-0"
              aria-label="Send"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-24 right-6 z-50 w-14 h-14 rounded-full bg-[#101F46] text-white shadow-xl flex items-center justify-center hover:scale-110 transition-transform"
      ...
      >
        ...
        {!open && messages.length > 1 && (
          <span className="absolute -top-1 -right-1 w-5 h-5 bg-amber-400 rounded-full text-[#101F46] text-xs font-bold flex items-center justify-center">
