        ...
        <section className="bg-white border-b border-[#e8e8e8]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
            <div className="flex items-baseline justify-between mb-6">
              <div>
                <h2 className="text-sm font-bold text-[#0B1F3A] uppercase tracking-[0.12em]">Notice Board</h2>
        ...
        <section className="bg-white pt-16 pb-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-baseline justify-between mb-8">
              <h2 className="text-xs font-bold text-[#0B1F3A] uppercase tracking-[0.15em]">Featured Story</h2>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 overflow-hidden rounded-lg border border-[#e8e8e8]">
                <div className="lg:col-span-3 relative min-h-[340px]">
                  ...
                  {featuredNews.category && (
                    <span className="absolute top-5 left-5 text-xs font-semibold text-[#0B1F3A] bg-amber-400 px-3 py-1.5 tracking-wide uppercase">
                      {featuredNews.category}
                    </span>
                  )}
                </div>
                <div className="lg:col-span-2 bg-[#0B1F3A] p-10 flex flex-col justify-center">
      ...
      {otherNews.length > 0 && (
        <section className="bg-[#f7f5f0] pt-10 pb-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-baseline justify-between mb-8">
              <div>
                <h2 className="text-xs font-bold text-[#0B1F3A] uppercase tracking-[0.15em]">Latest News</h2>
                <div className="w-6 h-0.5 bg-amber-400 mt-2" />
              </div>
                    ...
                    news.isFallback ? (
                      ...
                      >
                        ...
                        <div className="p-6 flex flex-col flex-1">
                          ...
                          <h3 className="font-bold text-[#0B1F3A] text-lg leading-snug mb-3 line-clamp-2">
                            {news.title}
                          </h3>
                          {news.description && (
                      ...
                      >
                        ...
                        <div className="p-6 flex flex-col flex-1">
                          ...
                          <h3 className="font-bold text-[#0B1F3A] text-lg leading-snug mb-3 line-clamp-2 group-hover:text-amber-600 transition-colors">
                            {news.title}
                          </h3>
                          {news.description && (
                          ...
                          <span className="mt-auto text-xs font-semibold text-[#0B1F3A] uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    news.isFallback ? (
                      ...
                      >
                        ...
                        <div className="p-5">
                          ...
                          <h3 className="font-bold text-[#0B1F3A] text-sm leading-snug line-clamp-2">{news.title}</h3>
                      ...
                      >
                        ...
                        <div className="p-5 flex-1">
                          ...
                          <h3 className="font-bold text-[#0B1F3A] text-sm leading-snug line-clamp-2 group-hover:text-amber-600 transition-colors">{news.title}</h3>
                          <span className="text-xs text-[#9ca3af] mt-2 block">{news.date}</span>
                        </div>
                      </Link>
                    )
              ...
              {otherNews.length === 1 && (
                otherNews[0].isFallback ? (
                  <div className="bg-white border border-[#e8e8e8] overflow-hidden max-w-md">
                    ...
                    <div className="p-6">
                      <h3 className="font-bold text-[#0B1F3A]">{otherNews[0].title}</h3>
                    </div>
                  </div>
                ) : (
                  <Link href={`/news/${otherNews[0].slug}`} className="group bg-white border border-[#e8e8e8] overflow-hidden max-w-md hover:shadow-lg transition">
                    ...
                    <div className="p-6">
                      <h3 className="font-bold text-[#0B1F3A] group-hover:text-amber-600 transition">{otherNews[0].title}</h3>
                    </div>
                  </Link>
                )
              )}
      ...
      {filteredNews.length === 0 && (
        <section className="py-28 bg-white">
          <div className="max-w-7xl mx-auto px-4 text-center">
            ...
            <h3 className="text-xl font-bold text-[#0B1F3A] mb-3">No stories found</h3>
            <p className="text-[#6b7280] mb-8 text-sm">Try adjusting your search or filter criteria.</p>
            <button
              onClick={() => { setSelectedCategory("All"); setSearchQuery(""); }}
              className="px-7 py-2.5 bg-[#0B1F3A] text-white text-sm font-semibold hover:bg-[#162d54] transition"
            >
              Clear filters
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <a
                ...
                className="px-6 py-3 bg-amber-400 text-[#0B1F3A] font-bold text-sm hover:bg-amber-300 transition-colors text-center"
              >
                Facebook
              </a>
              <a
