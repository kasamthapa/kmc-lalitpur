...
const inputCls = "w-full bg-white border border-gray-300 rounded-lg px-3 py-2.5 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400/20 focus:border-amber-500 transition-colors";
const labelCls = "block text-gray-500 text-xs font-semibold mb-1.5 uppercase tracking-wider";
...
export default function NewsPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-[60] bg-white border border-green-200 text-green-700 text-sm font-medium px-4 py-3 rounded-xl shadow-lg flex items-center gap-2 animate-in slide-in-from-top-2">
      ...
      <div className="flex items-start justify-between mb-7 gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Manage news articles and announcements.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          {!loading && <span className="text-gray-400 text-[10px]">({news.length})</span>}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {news.map((item) => (
              <div key={item.id} className="bg-white border border-gray-200 hover:border-gray-300 rounded-xl p-4 flex items-center gap-4 transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-gray-900 font-semibold text-sm">{item.title}</p>
                    {item.featured && (
                      <span className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded-full font-semibold">Featured</span>
                    )}
                    {item.category && (
                      <span className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">{item.category}</span>
                    )}
                  </div>
                  <p className="text-gray-400 text-xs font-mono mt-0.5">/news/{item.slug}</p>
      ...
      {showModal && (
        ...
        >
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-2xl my-8 shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <div>
                <h2 className="text-gray-900 font-bold text-base">{editingId ? "Edit Article" : "New Article"}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-5 pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={F.published} onChange={(e) => set("published", e.target.checked)} className="w-4 h-4 rounded border-gray-300 bg-white accent-amber-500" />
                  <span className="text-gray-500 text-sm">Published</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={F.featured} onChange={(e) => set("featured", e.target.checked)} className="w-4 h-4 rounded border-gray-300 bg-white accent-amber-500" />
                  <span className="text-gray-500 text-sm">Featured</span>
                </label>
                <div className="flex gap-2 ml-auto">
                  <button type="button" onClick={closeModal} disabled={saving} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 disabled:opacity-50 text-gray-700 text-sm font-semibold rounded-lg transition-colors">
                    Cancel
                  </button>
      ...
      {/* Delete confirmation */}
      {deleteId && (
        <div className="fixed inset-0 bg-gray-900/40 z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setDeleteId(null); }}>
          <div className="bg-white border border-gray-200 rounded-2xl p-6 max-w-sm w-full shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-gray-900 font-bold text-sm">Delete article?</h3>
                <p className="text-gray-400 text-xs mt-0.5">This cannot be undone.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg px-3 py-2.5 mb-5">
              <p className="text-gray-700 text-sm font-medium line-clamp-2">{deleteTitle}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setDeleteId(null)} className="flex-1 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-lg transition-colors">
                Cancel
              </button>
