...
export default function NoticesPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-7">
        <h1 className="text-gray-900 text-xl font-bold">Notices</h1>
        <p className="text-gray-400 text-sm mt-0.5">Manage the scrolling notice board on the homepage.</p>
      </div>

      {/* Add form */}
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-7">
        <h2 className="text-gray-700 text-sm font-bold mb-4">Add New Notice</h2>
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div>
            <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-wider mb-1.5">
              Notice Text <span className="text-red-500 normal-case">*</span>
            </label>
            <textarea
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-wider mb-1.5">Start Date</label>
              <input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} className={inputCls} />
            </div>
            <div>
              <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-wider mb-1.5">End Date</label>
              <input type="date" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} className={inputCls} />
            </div>
          </div>
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} className="w-4 h-4 rounded accent-amber-500" />
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">All Notices</h2>
          {!loading && <span className="text-gray-400 text-[10px]">({notices.length})</span>}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white border border-gray-200 rounded-xl">
            <p className="text-gray-400 text-sm">No notices yet. Add one above.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {notices.map((notice) => (
              <div
                ...
                className={`bg-white border rounded-xl p-4 flex items-start gap-4 transition-opacity ${
                  notice.active ? "border-gray-200" : "border-gray-100 opacity-50"
                }`}
              >
                <div className="flex-1 min-w-0">
                  <p className="text-gray-700 text-sm leading-snug">{notice.text}</p>
      ...
      {/* Delete confirmation */}
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40" onClick={(e) => { if (e.target === e.currentTarget) setDeleteTarget(null); }}>
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-gray-900 font-bold text-sm">Delete notice?</h3>
                <p className="text-gray-400 text-xs">This cannot be undone.</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg px-3 py-2.5 mb-5">
              <p className="text-gray-700 text-sm line-clamp-3">{deleteTarget.text}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setDeleteTarget(null)} disabled={deleting} className="flex-1 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-lg transition-colors">Cancel</button>
