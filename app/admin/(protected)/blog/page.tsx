    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-gray-600 text-[10px] font-bold uppercase tracking-widest">All Posts</h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {posts.map((post) => (
              <div
                key={post.id}
                className="bg-gray-900 border border-white/[0.06] rounded-xl p-4 flex items-start gap-4 hover:border-white/[0.1] transition-colors"
              >
                {post.imageUrl ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {post.featured && (
                      <span className="text-[10px] bg-amber-400/15 text-amber-400 border border-amber-400/20 font-bold px-2 py-0.5 rounded-full">Featured</span>
                    )}
                    {post.category && (
                      <span className="text-[10px] bg-white/[0.06] text-gray-500 px-2 py-0.5 rounded-full">{post.category}</span>
                    )}
                  </div>
                  <p className="text-gray-700 text-xs font-mono mt-1">/blog/{post.slug}</p>
                ...
                <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-white/[0.06] text-gray-300 hover:bg-white/[0.1] hover:text-white transition-colors"
                  >
                    Edit
                  </button>
                  <button
        ...
        >
          <div className="bg-gray-900 border border-white/[0.08] rounded-2xl w-full max-w-2xl my-8 shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
              <h2 className="text-white font-bold text-sm">{editingId ? "Edit Post" : "New Blog Post"}</h2>
              <button onClick={closeForm} className="text-gray-500 hover:text-white transition-colors p-1 rounded-lg hover:bg-white/[0.06]">
        ...
        >
          <div className="bg-gray-900 border border-white/[0.08] rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-bold text-sm">Delete post?</h3>
                <p className="text-gray-600 text-xs">This cannot be undone.</p>
              </div>
            </div>
            <div className="bg-gray-800 rounded-lg px-3 py-2.5 mb-5">
              <p className="text-gray-300 text-sm font-medium line-clamp-2">{deleteTarget.title}</p>
              {deleteTarget.author && <p className="text-gray-600 text-xs mt-0.5">{deleteTarget.author}</p>}
            </div>
